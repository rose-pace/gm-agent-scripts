...
class Skill(BaseModel):
    ...
    def validate_bonus(cls, v: int, values: Dict) -> int:
        ...
        prof_bonus = values.get('proficiency_bonus', 2)  # Default to 2 if not provided
...
class SavingThrow(BaseModel):
    ...
    def validate_bonus(cls, v: int, values: Dict) -> int:
        ability_mod = values.get('ability_modifier', 0)
        prof_bonus = values.get('proficiency_bonus', 2)
...
def calculate_proficiency_bonus(cr: float) -> int:
